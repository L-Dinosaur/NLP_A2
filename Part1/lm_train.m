...
for iFile=1:length(DD)
  ...
  for l=1:length(lines)
    ...
    for j=1:(length(words)-1)
        if(isfield(LM.bi, words{j}))
            if(isfield(LM.bi.(words{j}), words{j+1}))
                LM.bi.(words{j}).(words{j+1}) = LM.bi.(words{j}).(words{j+1}) + 1;
            else
                LM.bi.(words{j}).(words{j+1}) = 1;
            end
        else
            LM.bi.(words{j}) = struct();
            LM.bi.(words{j}).(words{j+1}) = 1;
        end
    end
